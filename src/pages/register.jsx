  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-stone-100 p-0 lg:p-12 font-sans">
      <div className="flex w-full max-w-7xl bg-white rounded-none lg:rounded-[2.5rem] shadow-none lg:shadow-2xl overflow-hidden min-h-screen lg:min-h-[750px]">
        ...
        {/* LADO IZQUIERDO: Formulario (Entra desde la izquierda en PC) */}
        <motion.div 
        ...
        >
          <div className="w-full max-w-md">
            ...
            <form className="space-y-5">
              <Input label="Nombre completo" type="text" placeholder="Ej: Mateo Giraldo" />
              <Input label="Correo electrónico" type="email" placeholder="mateo@ecoturistea.com" />
              <Input label="Contraseña" type="password" placeholder="••••••••" />
              
              <button className="w-full bg-green-700 text-white font-bold py-4 lg:py-5 rounded-2xl shadow-xl text-lg mt-4 lg:mt-8 hover:bg-green-800 transition-all">
                Registrarme ahora
              </button>
            </form>
        ...
        {/* LADO DERECHO: Imagen (Exactamente la misma del Login) */}
        <motion.div 
          initial={isPC ? { x: 150, opacity: 0 } : {}}
          animate={isPC ? { x: 0, opacity: 1 } : {}}
          transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}
        ...
        >
          ...
          <div className="absolute bottom-20 left-12 right-12 text-white z-20">
            <h3 className="text-5xl font-black mb-4 leading-tight drop-shadow-2xl">
              Explora los pulmones de Medellín
            </h3>
            <p className="text-xl text-green-50/90 max-w-xl font-medium drop-shadow-lg">
              Desde caminatas en Santa Elena hasta reservas protegidas en Envigado.
            </p>
          </div>
        </motion.div>
      </div>
